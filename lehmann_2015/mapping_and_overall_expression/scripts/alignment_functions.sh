...
function check_bowtie {
    ...
    if [[ $VAR -eq -1 ]]
    then
        echo $NAME "- WARNING: Bowtie input/output size inconsistent" >>$ALN_ERROR_LOG
    fi
...
function last_se_uniq {
    ...
    # PARSE LAST FILES
        ...
        perl /scratch/lfs/mcintyre/cegs/scripts/parse_last_sam_v2.pl \
            ${NAME}_last.sam \
...
function last_se_all {
    ...
    # READS is the location where the output files are placed
        ...
        python /bio/mcintyre/python.git/SeparateLast.py $READS $NUMPROCS
    ...
    # PARSE LAST FILES
        echo "START Parsing LAST SAM file into unique and ambiguous [`date`]" >>$MYLOG
        perl /bio/mcintyre/cegs/scripts/parse_last_sam_v2.pl \
            ${NAME}_last.sam \
            ${NAME}_ambig_last.sam \
            ${NAME}_uniq_last.sam \
            > $ALNLOGS/${NAME}_LAST.log
...
function last_pe_all {
    ...
    # READS is the location where the output files are placed
        ...
        python /bio/mcintyre/python.git/SeparateLast.py $READ1 $NUMPROCS
        rm $READ1
    ...
    # READS is the location where the output files are placed
        ...
        python /bio/mcintyre/python.git/SeparateLast.py $READ2 $NUMPROCS
        rm $READ2
