/* Counting splicing events for autoimmune genes */
...
data splicing.splicing_results_annot_immuno;
...
proc freq data=splicing.splicing_results_annot_immuno noprint;
...
data splicing.event_counts_summary_ai oops;
   merge all_event_counts (in=in1) sig_event_counts;
   by flag_CD4_on flag_CD8_on flag_CD19_on event_type;
   if in1 then output splicing.event_counts_summary_ai;
   else output oops;
run;
