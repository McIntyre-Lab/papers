...
proc sort data=fear.Cis_data_estiamtes;
by fusion_id;

data sig;
set fear.Cis_data_estiamtes;
if flag_all_ai=1;
run;

proc sort data=fear.Fb551_si_fusions_unique_flagged;
by fusion_id;

*count direction of AI within a line mating status;
data sig_anno;
merge sig (in=in1) fear.Fb551_si_fusions_unique_flagged(in=in2);
by fusion_id;
if in1;
