/*******************************************************************************
* Filename: cnt_snp_diffs_between_w1118_and_line_jmf4.sas
*
...
%macro count_w11182line_snps(ID);
    ...
    proc sort data = WORK.w1118_vcf;
        by chrom pos;
        run;

    proc sort data = WORK.&ID._vcf;
        by chrom pos;
        run;

    data w1118_2_&ID._both  w1118_2_&ID._not_both  ;
        merge WORK.w1118_vcf (in=in1)  WORK.&ID._vcf (in=in2);
        by chrom pos;
    ...
    data w1118_2_&ID._both2 ;
        ...
        if Alt_&ID = ' ' then alt2_&ID = ref_&ID ;   /* if line snp is missing then is same*/ 
		/* as ref base */
        else alt2_&ID = alt_&ID ;
        if Alt_w1118 = ' ' then alt2_w1118 = ref_w1118 ;   
	/* if w1118 snp is missing then is same as ref base */
    ...
    data w1118_2_&ID._cnts;
        ...
        if w1118_ishet = 1 or &ID._ishet = 1 then snp_present = 0;  
		/* if w1118 is a het or line is a het then snp not present */
        else if alt2_w1118 ne alt2_&ID then snp_present = 1 ;       
		/* if w1118 snp is not the same as the line snp then snp is present */
        if w1118_snp = 1 then snp_present = 1 ;                 
		/* if there is a w1118 snp then the snp is present  */
        if &ID._snp = 1 then snp_present = 1 ;                 
		/* if there is a line snp then the snp is present */
        else snp_present = 0;
...
data THUMP.snp_diffs_w1118_2_line_completes ;
...
/*
proc export data = THUMP.snp_diffs_w1118_2_line
