...
x1=xs[which(js==1)]; if(sum(x1)==0)	x1[sample(1:I1,1)]=0.5;
y1=ys[which(js==1)]; if(sum(y1)==0)	y1[sample(1:I1,1)]=0.5;
z1=zs[which(js==1)]; if(sum(z1)==0)	z1[sample(1:I1,1)]=0.5;
x2=xs[which(js==2)]; if(sum(x2)==0)	x2[sample(1:I2,1)]=0.5;
y2=ys[which(js==2)]; if(sum(y2)==0)	y2[sample(1:I2,1)]=0.5;
z2=zs[which(js==2)]; if(sum(z2)==0)	z2[sample(1:I2,1)]=0.5;
#Checking that I have not a vector of all zeros (this messes with the convergence, if so I add a 1 at random)

if(sum(z1)==0)	z1[sample(1:I1)]=1
if(sum(z2)==0)	z2[sample(1:I2)]=1
if(sum(x1)==0)	x1[which.max(z1)]=1
if(sum(x2)==0)	x2[which.max(z2)]=1
if(sum(y1)==0)	y1[which.max(z1)]=1
if(sum(y2)==0)	y2[which.max(z2)]=1
