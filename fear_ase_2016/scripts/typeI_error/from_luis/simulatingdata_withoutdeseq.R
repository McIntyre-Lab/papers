...
out$RNA_Sim_all_Rep3=out$RNA_Sim_all_Rep2=out$RNA_Sim_all_Rep1=
out$RNA_Mel_all_Rep3=out$RNA_Mel_all_Rep2=out$RNA_Mel_all_Rep1=rep(NA,nrow(out))
names(out)

for(i in 1:nrow(out)){
	out[i,c("RNA_Mel_all_Rep1","RNA_Mel_all_Rep2","RNA_Mel_all_Rep3")]=#rnbinom(3, size=1/out$trueoverdispersion[i],  mu=(1-out$q_true)[i]*out$truemeanM[i])
	rpois(3,(1-out$q_true)[i]*out$truemeanM[i])

	out[i,c("RNA_Sim_all_Rep1","RNA_Sim_all_Rep2","RNA_Sim_all_Rep3")]=#rnbinom(3, size=1/out$trueoverdispersion[i],  mu=out$q_true[i]*out$truemeanS[i])
	rpois(3,out$q_true[i]*out$truemeanS[i])
	}
par(mfrow=c(2,1))
...
write.csv(out,file=paste("./data/simulated_",scenario,"Requal",fortitle,"Poisson_q_truefirst5000_0d",q_truefirst5000*100,".csv",sep=""),quote=FALSE,row.names = FALSE)
}else{# If scenario==	"NOAI_NOBIAS" we also generate DNA counts
##Creating the CountDataSet object, central data structure of DESeq
...
out_DNA$DNA_Sim_all_Rep3=out_DNA$DNA_Sim_all_Rep2=out_DNA$DNA_Sim_all_Rep1=
out_DNA$DNA_Mel_all_Rep3=out_DNA$DNA_Mel_all_Rep2=out_DNA$DNA_Mel_all_Rep1=rep(NA,nrow(out))
head(out_DNA)

for(i in 1:nrow(out)){
	out_DNA[i,c("DNA_Mel_all_Rep1","DNA_Mel_all_Rep2","DNA_Mel_all_Rep3")]=#rnbinom(3, size=1/out_DNA$trueoverdispersion_DNA[i],  mu=(1-out_DNA$q_true_DNA[i])*out_DNA$truemeanM_DNA[i])
	rpois(3,(1-out_DNA$q_true_DNA[i])*out_DNA$truemeanM_DNA[i])
	
	out_DNA[i,c("DNA_Sim_all_Rep1","DNA_Sim_all_Rep2","DNA_Sim_all_Rep3")]=
	#rnbinom(3, size=1/out_DNA$trueoverdispersion_DNA[i],  mu=out_DNA$q_true_DNA[i]*out_DNA$truemeanS_DNA[i])
	rpois(3,out_DNA$q_true_DNA[i]*out_DNA$truemeanS_DNA[i])
	}
...
write.csv(cbind(out,out_DNA),file=paste("./data/simulated_",scenario,"Requal",R,"Poisson_q_truefirst5000_0d",q_truefirst5000*100,".csv",sep=""),quote=FALSE,row.names = FALSE)
}
