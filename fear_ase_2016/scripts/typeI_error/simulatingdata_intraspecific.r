...
if(scenario != "NOAI_NOBIAS"){
    write.csv(out,file=paste("./r101_simulated_",scenario,"Requal",fortitle,"Poisson_q_truefirst5000_0d",q_truefirst5000*100,".csv",sep=""),quote=FALSE,row.names = FALSE)
} else{
    # If scenario == "NOAI_NOBIAS" we also generate DNA counts
    countsTable_DNA <- mydata[c("LINE_DNA_TOTAL","TESTER_DNA_TOTAL")]
    ...
    sumM_DNA <- countsTable_DNA[,c("LINE_DNA_TOTAL")]
    sumS_DNA <- countsTable_DNA[,c("TESTER_DNA_TOTAL")]
    ...
    out_DNA$tester_DNA_Rep3 <- out_DNA$tester_DNA_Rep2 <- out_DNA$tester_DNA_Rep1 <- out_DNA$line_DNA_Rep3 <- out_DNA$line_DNA_Rep2 <- out_DNA$line_DNA_Rep1 <- rep(NA,nrow(out))
    head(out_DNA)

    for(i in 1:nrow(out)){
        out_DNA[i,c("line_DNA_Rep1","line_DNA_Rep2","line_DNA_Rep3")] <- rpois(3,(1-out_DNA$q_true_DNA[i])*out_DNA$truemeanM_DNA[i])
        
        out_DNA[i,c("tester_DNA_Rep1","tester_DNA_Rep2","tester_DNA_Rep3")] <- rpois(3,out_DNA$q_true_DNA[i]*out_DNA$truemeanS_DNA[i])
    }
        
    par(mfrow=c(2,1))
    sumM_DNA <- rowSums(out_DNA[,c("line_DNA_Rep1","line_DNA_Rep2","line_DNA_Rep3")])
    ...
    #write.csv(cbind(out,out_DNA),file=paste("./r101_simulated_",scenario,"Requal",R,"Poisson_q_truefirst5000_0d",q_truefirst5000*100,".csv",sep=""),quote=FALSE,row.names = FALSE)
    write.csv(cbind(out,out_DNA),file=paste("./r101_simulated_",scenario,"Requal",R,"Poisson.csv",sep=""),quote=FALSE,row.names = FALSE)
}
