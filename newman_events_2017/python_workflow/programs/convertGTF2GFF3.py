...
def main():
    ...
    with open(args.outputFile, 'wb') as outputFile:
        ...
        for gene in genes:
            ...
            geneID=gene.id
            try:
                geneName=''.join(gene.attributes['Name'])
            except:
                geneName=gene.id
            ...
            geneEntry=[geneChrom, geneSource, 'gene', str(geneStart), str(geneStop), '.', geneStrand, '.', "ID=" + str(gene.id) + ";Name=" + str(geneName)]
            outputFile.write("\t".join(geneEntry) + "\n")
            
            # Format transcripts and CDS
            xscriptList = list(db.children(gene, featuretype=('transcript','mRNA','ncRNA','miRNA','pre_miRNA','pseudogene','rRNA','snoRNA','snRNA','tRNA'), order_by=('start','end')))
            ...
            for xscript in xscriptList:
                ...
                try:
                    xscriptName=''.join(xscript.attributes['Name'])
                except:
                    xscriptName=xscript.id
                
                # Format GFF entry and append to GFF list
                xscriptEntry=[geneChrom, geneSource, str(xscript.featuretype), str(xscriptStart), str(xscriptStop), '.', geneStrand, '.', "ID=" + xscriptID + ";Name=" + xscriptName + ";Parent=" + geneID]
                ...
                for cds in cdsList:
                    ...
                    cdsStop=cds.end
                    cdsFrame=cds.frame
                    cdsID=str(xscript.id) + "-cds"
                    
                    #Format GFF entry and append to GFF list
                    cdsEntry=[geneChrom, geneSource , 'CDS' , str(cdsStart), str(cdsStop), '.', geneStrand, str(cdsFrame), "Name=" + cdsID + ";Parent=" + xscriptID + ";parent_type=transcript"]
                ...
                for intron in intronList:
                    ...
                    intronID=str(xscript.id) + "-in" + str(intronIndex)
                    intronIndex=intronIndex+1 # Append an intron number for each intron in a transcript
                    
                    # Format GFF entry and append to GFF list
                    intronEntry=[geneChrom, geneSource, 'intron', str(intronStart), str(intronStop), '.', geneStrand, '.', "Name=" + intronID + ";Parent=" + xscriptID + ";parent_type=transcript"]
            ...
            for exon in exonList:
                ...
                if index == 1: # If first exon in a gene get all the required pieces
                    ...
                    exonStop=exon.end
                    parentList = list(db.parents(exon, featuretype=('transcript','mRNA','ncRNA','miRNA','pre_miRNA','pseudogene','rRNA','snoRNA','snRNA','tRNA'), order_by='start'))
                    exonParents = []
                    ...
                    if (exonStart == currExonStart and exonStop == currExonStop):
                        ...
                        exonEntry=[geneChrom, geneSource, 'exon', str(currExonStart), str(currExonStop), '.', geneStrand, '.', "Name=" + currExonID + ";Parent=" + currExonParentsString + ";parent_type=transcript"]
