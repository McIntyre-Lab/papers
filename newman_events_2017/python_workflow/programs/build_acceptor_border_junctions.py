...
##        with exonic regions, For plus-strand genes, retained introns are made from the start of exons. For
...
def main():
    ...
    for gene in genes:
        ...
        if len(exonList) == 0:
            print str(gene.id) + " has no exons"
            continue
            ...
            if index == 1:
                ...
                if gene.strand == '+':
                    if (exonStop >= currExonStart):  # If exon overlaps with current exon
                        ...
                        exonEntry = '\t'.join(
                            [str(eventChrom), str(totalStart), str(totalStop), eventName, eventScore, eventStrand,
                             color, blocks, blockLength, blockStarts]) + "\n"
                        ...
                        exonListOut = '|'.join(exonNameList)
                        csvEntry = ','.join(
                            [eventGene, eventName, str(eventChrom), eventStrand, str(intronPos), currExonID,
                             exonListOut, isLastExon]) + "\n"
