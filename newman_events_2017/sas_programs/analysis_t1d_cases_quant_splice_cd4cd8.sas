...
data genes4test;
   set event.hg19_genes_for_ds_test_cd4cd8;
...
data event.t1d_quant_splice_anova_cd48;
   set anova;
run;

data event.t1d_quant_splice_resid_cd48;
   set resid;
run;

data event.t1d_quant_splice_lsmeans_cd48;
   set lsmeans;
run;

data event.t1d_quant_splice_flag_p05_cd48;
   set flag_sig_int;
run;
