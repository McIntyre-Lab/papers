...
proc freq data=flag_partial2 noprint;
  tables flaG_perc_ident_ge95*flag_no_mismatch*flag_no_gap*flag_feature_on_ge10 / out=check;
...
data event_dtct;
   set event.flag_feature_on_all_reps_ge5;
   keep feature_id flag_feature_on_ge10;
   rename feature_id=event_id;
run;
...
proc means data=flag_length2 noprint;
   ...
   var flag_event_has_blast flag_pb_gene_has_refseq flag_junction_annotated flag_intron_retention
         ...
         flag_mean_apn_ge5 flag_feature_on_ge10;
...
data good_blast ;
  ...
  and flag_feature_on=1 and flag_hit_90perc_len=1 and flag_feature_on_ge10=1 ;
  keep event_id;
run;
...
data event.blast_junc_to_pb_summary_apn10;
  set blast_summary;
run;


data event.blast_junc_to_pb_all_hits_apn10;
  set flag_length;
run;
...
proc freq data=hits_w_annot noprint;
   tables flag_junction_annotated*flag_intron_retention*flag_feature_on_ge10*flag_event_has_hit / out=blast_check;
run;
...
data event.blast_junc_to_pb_w_annot_apn10;
  set hits_w_annot;
run;
