...
I am going to do this on only the set of detected splicing events after gene removal
...
proc sort data=event2keep nodup;
   by event_id;
...
data events_for_bed;
   merge events_on (in=in1) event2coord (in=in2) event2keep (in=in3);
   by event_id;
...
proc export data=assemble_bed
        outfile='!MCLAB/event_analysis/references/refseq_mm10_detected_splicing_events_nsc_nomulti.bed'
        dbms=tab replace;
        putnames=no;
        run;
