...
proc sort data=mean_counts2;
   by gene_id;
proc means data=mean_counts2 noprint;
   by gene_id;
   var logapn_CD19 logapn_CD4 logapn_CD8;
   output out=max_frag max(logapn_CD19)=max_CD19
                       max(logapn_CD4)=max_CD4
                       max(logapn_CD8)=max_CD8;
run;

proc sort data=mean_counts2;
  by gene_id;
proc sort data=max_frag;
  by gene_id;
run;

data mean_counts3;
  merge max_frag (in=in1) mean_counts2 (in=in2);
  by gene_id;
  if in1 and in2;
  drop _TYPE_ _FREQ_;
run;
...
data mean_counts4;
  set mean_counts3;
  logapn_CD19_scaled=logapn_cd19/max_cd19;
  logapn_CD4_scaled=logapn_cd4/max_cd4;
  logapn_CD8_scaled=logapn_cd8/max_cd8;
  drop max_cd19 max_cd4 max_cd8 logapn_cd19 logapn_cd4 logapn_cd8;
run;
...
proc sort data=mee_sbys2;
   by fragment_name;
proc sort data=mean_counts4;
   by fragment_name;
run;

data export_counts_mee;
  merge mean_counts4 (in=in1) mee_sbys2 (in=in2);
   by fragment_name;
...
data export_counts_mee2;
  retain gene_id fusion_strand fragment_name logapn_cd19_scaled
         logapn_cd4_scaled logapn_cd8_scaled flag_mee_cd19 flag_mee_cd4 flag_mee_cd8;
...
proc export data=export_counts_mee2 outfile="!MCLAB/event_analysis/analysis_output/t1d_cases_genes_mei_and_mee_fragments_scaled.csv"
   dbms=csv replace;
run;
