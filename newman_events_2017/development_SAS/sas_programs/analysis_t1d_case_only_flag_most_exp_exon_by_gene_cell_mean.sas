...
/* Calculate the mean coverage for each fusion by cell type */

proc sort data=counts_w_key_subset;
   by gene_id fusion_id cell_type;
proc means data=counts_w_key_subset noprint;
   by gene_id fusion_id cell_type;
   var log_q3_q3_apn;
   output out=mean_logapn_by_cell mean=;
run;
...
proc sort data=mean_logapn_by_cell;
   by gene_id cell_type descending log_q3_q3_apn fusion_id;
run;

data flag_mee_by_cell;
  set mean_logapn_by_cell;
...
proc sort data=flag_mee_by_cell;
  by gene_id fusion_id cell_type;
run;

proc transpose data=flag_mee_by_cell out=mee_by_cell_sbys;
   by gene_id fusion_id;
   id cell_type;
   var flag_mee;
run;

proc transpose data=flag_mee_by_cell out=apn_by_cell_sbys;
   by gene_id fusion_id;
   id cell_type;
   var log_q3_q3_apn;
run;

proc transpose data=flag_mee_by_cell out=rank_by_cell_sbys;
   by gene_id fusion_id ;
   id cell_type;
   var exon_rank;
run;
...
data eventloc.t1d_exons_mee_rank_by_cell_mean;
  merge apn_by_cell_sbys2 mee_by_cell_sbys2 rank_by_cell_sbys2;
  by gene_id fusion_id ;
run;

/* Count: fusions that are diff */

proc freq data=eventloc.t1d_exons_mee_rank_by_cell_mean noprint;
  tables flag_mee_cd19*flag_mee_cd4*flag_mee_cd8 / out=mee_count;
proc print data=mee_count;
run;quit;


/*
   flag_      flag_      flag_
 mee_cd19    mee_cd4    mee_cd8    COUNT
...
/* Now count genes with MEE differences */

data cd4 cd8 cd19;
   set eventloc.t1d_exons_mee_rank_by_cell_mean;
   if flag_mee_cd19=1 then output cd19;
   if flag_mee_cd4=1 then output cd4;
   if flag_mee_cd8=1 then output cd8;
   keep gene_id fusion_id;
run;

data cd4_2;
  set cd4;
  rename fusion_id=cd4_mee_fusion_id;
run;

data cd8_2;
  set cd8;
  rename fusion_id=cd8_mee_fusion_id;
run;

data cd19_2;
  set cd19;
  rename fusion_id=cd19_mee_fusion_id;
run;
...
data flag_gene_mee_diff;
  merge cd4_2 cd8_2 cd19_2;
  by gene_id;
run;

data flag_gene_mee_diff2;
  set flag_gene_mee_diff;
  if cd4_mee_fusion_id=cd8_mee_fusion_id then flag_cd4cd8_mee_diff=0; else flag_cd4cd8_mee_diff=1;
  if cd4_mee_fusion_id=cd19_mee_fusion_id then flag_cd4cd19_mee_diff=0; else flag_cd4cd19_mee_diff=1;
  if cd8_mee_fusion_id=cd19_mee_fusion_id then flag_cd8cd19_mee_diff=0; else flag_cd8cd19_mee_diff=1;
run;

proc freq data=flag_gene_mee_diff2 noprint;
 tables flag_cd4cd8_mee_diff*flag_cd4cd19_mee_diff*flag_cd8cd19_mee_diff / out=mee_diff_check;
proc print data=mee_diff_check;
run;
quit;

/*
 flag_cd4cd8_    flag_cd4cd19_    flag_cd8cd19_
   mee_diff         mee_diff         mee_diff      COUNT
...
data eventloc.t1d_mee_comparison_by_gene;
  set flag_gene_mee_diff2;
run;
