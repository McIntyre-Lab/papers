...
data flag_counts;
   *set eventloc.hg19_counts_by_fragment;
   ...
   if apn gt 0 then flag_frag_on=1;
   else flag_frag_on=0;
run;
...
proc means data=flag_counts noprint;
   by fragment_id cell_type;
   var flag_frag_on;
   output out=perc_on_by_trt mean=;
run;
...
data flag_fragment_on_apn0;
   merge flag_cd4 flag_cd8 flag_cd19;
   by fragment_id;
   if flag_cd4_on=1 and flag_cd8_on=1 and flag_cd19_on=1 then flag_fragment_all_on_gt0=1;
   else flag_fragment_all_on_gt0=0;
run;

/* Make permenant */

data event.hg19_flag_fragment_on_apn0;
   set flag_fragment_on_apn0;
run;
