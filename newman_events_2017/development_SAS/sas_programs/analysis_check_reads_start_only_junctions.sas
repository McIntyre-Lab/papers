...
proc import datafile="!MCLAB/event_analysis/analysis_output/read_check_output.csv"
...
/*
   ...
   --------------------------------------------------------------------
                 0         325       65.92           325        65.92
                 1         168       34.08           493       100.00

A third of all extracted reads overlay multiple splice junctions
...
/*
  ...
  flag_multi_junc    Frequency     Percent     Frequency      Percent
  --------------------------------------------------------------------
                0         126       85.14           126        85.14
                1          22       14.86           148       100.00

22 of 148 STAR-only junctions are due to the read mapping to multiple junction sites
...
---------------------------------------------------------------------
               0           1        0.60             1         0.60
               1         167       99.40           168       100.00

All but one match to a known microexon
*/
...
One read with an insertion, one with a deletion
...
/*
 ...
 insertion    deletion      indel     COUNT
...
proc sort data=annot nodup;
   by junction_id;
proc sort data=nonpb_juncs_star;
   by junction_id;
run;

data nonpb_junc_star_type;
  merge nonpb_juncs_star (in=in1) nic (in=in2) annot (in=in3);
  by junction_id;
...
proc freq data=nonpb_junc_star_type noprint;
...
/*
 microdonor    microacceptor    microexon    indel    annot    flag_nic    COUNT
