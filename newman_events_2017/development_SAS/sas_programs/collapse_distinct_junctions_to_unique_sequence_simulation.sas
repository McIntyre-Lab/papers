...
/* Import distinct junctions FASTA and collapse on sequence */
    ...
    infile '!MCLAB/event_analysis/references/simulation_distinct_junc_coord.fa'
...
proc sort data=seq_count;
  by descending count;
run;
*2951862 distinct coordinates, 2933616 unique sequences (strand specific);
...
/* Okay, I DON'T need to flip the sequence. I am going to export the set of 2933616 unique junction sequences
...
data evspl.simulation_junc2uniq_seq;
   set junc2uniq_seq;
...
proc export data=junc_uniq_seq5
     outfile="!MCLAB/event_analysis/references/simulation_unique_junction_seq.fa"
     dbms=tab replace;
     putnames=no;
run;
