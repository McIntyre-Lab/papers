...
data mee lee;
   ...
   if first.subject_id then output mee;
   else if last.subject_id then output lee;
   keep gene_id cell_type subject_id log_q3_q3_apn;
run;

proc sort data=mee;
   by gene_id cell_type subject_id log_q3_q3_apn;
proc sort data=lee;
   by gene_id cell_type subject_id log_q3_q3_apn;
proc sort data=counts_w_key_subset;
   by gene_id cell_type subject_id log_q3_q3_apn;
...
data flag_mee_by_subj_cell;
   merge counts_w_key_subset (in=in1) mee (in=in2) lee (in=in3);
   by gene_id cell_type subject_id log_q3_q3_apn;
   if in2 then do;
        exon_rank=1;
        flag_mee=1;
...
data eventloc.t1d_exons_mee_rank_cell_subj_v2;
