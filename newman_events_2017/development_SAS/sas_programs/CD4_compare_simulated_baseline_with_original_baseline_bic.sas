...
%macro combine_bic();
    %do i=1 %to &numsims; 
        libname addgen "/mnt/store/grn_sandbox/cd4_8000_baseline_simulation/8000_gene_sim/&i./sas_data";
...
proc printto log="/mnt/store/grn_sandbox/CD4_100_sim_combine.log" new; run;
%combine_bic();
...
data diff;
   set combined;
   genecov_bic=1762.5546;
   fullcov_bic=1858.8794;
   genecov_diff=genecov_bic - bic;
   fullcov_diff=fullcov_bic - bic;
run;

ods graphics on;
ods pdf file="!MCLAB/grants/ase_grn/INSR-mTOR_T1D/analysis_output/CD4_100simulations_Baseline_only_sim_bic_dist_fullcovar.pdf";
