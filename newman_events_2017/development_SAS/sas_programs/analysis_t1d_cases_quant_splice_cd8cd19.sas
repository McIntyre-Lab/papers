...
data genes4test;
   set event.hg19_genes_for_ds_test_cd8cd19;
...
data event.t1d_quant_splice_anova_cd819;
   set anova;
run;

data event.t1d_quant_splice_resid_cd819;
   set resid;
run;

data event.t1d_quant_splice_lsmeans_cd819;
   set lsmeans;
run;

data event.t1d_quant_splice_flag_p05_cd819;
   set flag_sig_int;
run;
