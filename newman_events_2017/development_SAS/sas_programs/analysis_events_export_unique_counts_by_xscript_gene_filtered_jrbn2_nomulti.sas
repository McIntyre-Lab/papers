...
%count_features(feat_data_w_xs,transcript,feature_cnt_by_xscript_gene_exp);
%count_features(feat2gene,gene,feature_cnt_by_gene_exp);

/* Export data */

proc export data=event.feature_cnt_by_xscript_gene_exp
     outfile="!MCLAB/event_analysis/analysis_output/event_analysis_uniq_features_by_xscript_gene_filtered_nomulti.csv" dbms=csv replace;
run;

proc export data=event.feature_cnt_by_gene_exp
     outfile="!MCLAB/event_analysis/analysis_output/event_analysis_uniq_features_by_gene_gene_filtered_nomulti.csv" dbms=csv replace;
run;
