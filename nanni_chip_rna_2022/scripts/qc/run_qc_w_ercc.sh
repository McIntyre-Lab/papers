...
################################################################################################
#
#  Title: MCINTYRE LAB FASTQ QC PIPELINE (WITH ERCC CONTROLS), SLURM VERSION
...
#
#  NOTE: This version of the script will align reads to ERCC spike-in control sequences (mix 1). 
#  If you do not want to run ERCC alignments (for example, you have DNA sequences, or ERCCs
#  weren't added, you should review and run the run_qc_no_ercc.sh script.
#
#################################################################################################
...
EMAIL=jrbnewman@ufl.edu

# Set the project folder
PROJ=/ufrc/mcintyre/share/cegs2.0

# Set location of original data. This is the location containing the FASTQ/FASTQ.GZ files
ORIG=$PROJ/original_data/Project_Alison_Pool3
...
SLURMLOGSDIR=$PROJ/scripts/PBS_LOGS
...
MAIN_DESIGN=$PROJ/design_files/design_plate3.csv
...
# in the user-created design file.
...
# USER-DEFINED DESIGN VARIABLES (you will likely need to change this!)
    ...
    FASTQ_FILE=${RGCODE}_${BARCODE_I7}-${BARCODE_I5}
    FASTQ_LABEL=${RGCODE}_${GN}_${SEX}_${REP}
...
# Align SE to ERCC controls
JOB9_ID=$(sbatch --dependency=afterany:${JOB8_ID/Submitted batch job /} ${JOB9_SCRIPT})
echo "${JOB9_SCRIPT} submitted: Job name is ${JOB9_NAME} and the JobID is ${JOB9_ID}"
sleep 2

# Mpileups for ERCC controls
JOB10_ID=$(sbatch --dependency=afterany:${JOB9_ID/Submitted batch job /} ${JOB10_SCRIPT})
echo "${JOB10_SCRIPT} submitted: Job name is ${JOB10_NAME} and the JobID is ${JOB10_ID}"
sleep 2

# ERCC coverage counts
JOB11_ID=$(sbatch --dependency=afterany:${JOB10_ID/Submitted batch job /} ${JOB11_SCRIPT})
echo "${JOB11_SCRIPT} submitted: Job name is ${JOB11_NAME} and the JobID is ${JOB11_ID}"
sleep 2

# ERCC concentration plots
JOB12_ID=$(sbatch --dependency=afterany:${JOB11_ID/Submitted batch job /} ${JOB12_SCRIPT})
echo "${JOB12_SCRIPT} submitted: Job name is ${JOB12_NAME} and the JobID is ${JOB12_ID}"
sleep 2
