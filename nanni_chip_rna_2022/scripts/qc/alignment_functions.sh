...
function check_bowtie {
    ...
    VAR=`/ufrc/mcintyre/share/qc_pipeline/scripts/SizeChecker.py -i $READS -aln $ALN -unaln $UNALN`
...
function last_se_uniq {
    ...
    # READS is the location where the output files are placed
        ...
        python /scratch/lfs/jnewman/SeparateLast.py $READS $NUMPROCS
        rm $READS
...
function last_pe_uniq {
    ...
    # READS is the location where the output files are placed
        ...
        python /scratch/lfs/jnewman/SeparateLast.py $READ1 $NUMPROCS
    ...
    # READS is the location where the output files are placed
        ...
        python /scratch/lfs/jnewman/SeparateLast.py $READ2 $NUMPROCS
...
function last_pe_all {
    ...
    # PARSE LAST FILES
        echo "START Parsing LAST SAM file into unique and ambiguous [`date`]" >>$MYLOG
        perl /scratch/lfs/jnewman/parse_last_sam_v2.pl \
            ${NAME}_last.sam \
            ${NAME}_ambig_last.sam \
            ${NAME}_uniq_last.sam \
            > $ALNLOGS/${NAME}_LAST.log
