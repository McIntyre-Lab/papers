...
%macro out_loop(type);
    %macro letter_loop(letter);
        proc freq data=ind_rep_motif (where=(flag_null_&type = 1)) noprint; 
    ...
    proc export data=freq_motif
                outfile="!MCLAB/arbeitman/arbeitman_fru_network/reports_internal/motif_analysis/null_male_&type._motif_distribution.csv"
                dbms=CSV replace;
                putnames=yes;
                run;
