 ...
 */
proc sort data=fru.foranalysis_with_multi_miss;
    by fusion_id;
    run;

proc freq data=fru.foranalysis_with_multi_miss;
    tables trt;
    run;
...
proc glm data=fru.foranalysis_with_multi_miss plots=none;
    by fusion_id;
    class trt;
    model logrpkm = trt;
    lsmeans trt /pdiff;
    ...
    output out=fru.resid_by_fusion_miss r=resid p=pred;
    ods output modelanova=model_fru
               contrasts=fru.contrast_by_fusion_miss
               lsmeans=fru.lsmeans_by_fusion_miss
               means=fru.means_by_fusion_miss;
    run;
    quit;

ods listing;

data fru.model_fru_by_fusion_miss;
    set model_fru;
