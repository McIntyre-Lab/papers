...
data foranalysis;
    merge short_list2 (in=in1) 
          fru.flag_no_var_miss (in=in2) 
          fru.flag_zero_mean_miss (in=in3)
          fusion2gene (in=in4);
    by fusion_id;
    if in1 and in2 and in3 and in4;
    run; * 2291058 obs;

data fru.foranalysis_no_multi_miss;
    retain symbol_cat fusion_id;
    set foranalysis;
    where multigene = 0 and flag_no_var =0 and flag_zero_mean = 0;
    drop flag_no_var flag_zero_mean multigene;
    run; *1723034 obs;

data fru.foranalysis_with_multi_miss;
    retain symbol_cat fusion_id;
    set foranalysis;
