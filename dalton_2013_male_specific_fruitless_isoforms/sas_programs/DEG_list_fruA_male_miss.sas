...
data mydata;
    set fru.results_plus_gov2_miss;
    run;

/* Create A subset that removes un-analyzed fusions and multigene fusions */
    data subset;
        set mydata;
        if flag_fdr_p_contrast_11_20 = ' ' then delete;
        if flag_fdr_p_contrast_12_20  = ' ' then delete;
        where FBgn_cat ^? ';';
        run;
...
/* Identify Fusions that have sig FDR for all comparisons */
    data flag_fdr_all;
        set subset;
        if flag_fdr_p_contrast_11_20 = 1 and flag_fdr_p_contrast_12_20 = 1 then flag_fdr_all = 1; else flag_fdr_all= 0;
...
/* Identify Fusions that have sig FOLD CHANGE for all comparisons */
    data fold;
        set subset;
        fold_CS =  AH_CS_mean_rpkm / AH_MALE_FruM_A__mean_rpkm;
        fold_BER = AH_BerM_mean_rpkm / AH_MALE_FruM_A__mean_rpkm;
...
proc export data=induced
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/exported_data_from_michelle/FruM_A_male_ind_jmf_miss.tab'
            dbms=CSV replace;
            putnames=no;
            run;

proc export data=repressed
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/exported_data_from_michelle/FruM_A_male_rep_jmf_miss.tab'
            dbms=CSV replace;
            putnames=no;
            run;

data FRU.fruA_male_fusions_ind_rep_miss;
    set merged2;
    rename flag_induced = flag_fruA_male_ind;
    rename flag_repressed = flag_fruA_male_rep;
    keep fusion_id flag_induced flag_repressed;
    run;

data FRU.fruA_male_FBgn_ind_rep_miss;
    set induced (in=in1) repressed(in=in2);
    if in1 then flag_ind = 1; else flag_ind = 0;
    if in2 then flag_rep = 1; else flag_rep = 0;
    run;
