...
/* Create A subset that removes un-analyzed fusions and multigene fusions */
    data subset;
        set mydata;
        if flag_fdr_p_contrast_19_20 = ' ' then delete;
        if flag_fdr_p_contrast_20_20  = ' ' then delete;
        where FBgn_cat ^? ';';
        run;
...
/* Identify Fusions that have sig FDR for all comparisons */
    data flag_fdr_all;
        set subset;
        if flag_fdr_p_contrast_19_20 = 1 and flag_fdr_p_contrast_20_20 = 1 then flag_fdr_all = 1; else flag_fdr_all= 0;
...
/* Identify Fusions direction of change for all comparisons */
    data fold;
        set subset;
        fold_CS =  AH_CSFemale_mean_rpkm / AH_FEMALE_FruM_B__mean_rpkm;
        fold_BER = AH_BerF_mean_rpkm / AH_FEMALE_FruM_B__mean_rpkm;
...
proc export data=induced
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/exported_data_from_michelle/FruM_B_female_ind_1fold_jmf.tab'
            dbms=CSV replace;
            putnames=no;
            run;

proc export data=repressed
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/exported_data_from_michelle/FruM_B_female_rep_1fold_jmf.tab'
            dbms=CSV replace;
            putnames=no;
            run;

data FRU.fruB_female_fus_ind_rep_nfold;
    set merged2;
    rename flag_induced = flag_fruB_female_ind_nofold;
    rename flag_repressed = flag_fruB_female_rep_nofold;
    keep fusion_id flag_induced flag_repressed;
    run;

proc export data=FRU.fruB_female_fus_ind_rep_nfold
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/reports_external/FruM_B_female_flag_ind_rep_nofold.csv'
            dbms=CSV replace;
            putnames=yes;
            run;
