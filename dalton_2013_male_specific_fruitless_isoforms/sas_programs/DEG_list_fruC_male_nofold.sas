...
/* Create A subset that removes un-analyzed fusions and multigene fusions */
    data subset;
        set mydata;
        if flag_fdr_p_contrast_15_20 = ' ' then delete;
        if flag_fdr_p_contrast_16_20  = ' ' then delete;
        where FBgn_cat ^? ';';
        run;
...
/* Identify Fusions that have sig FDR for all comparisons */
    data flag_fdr_all;
        set subset;
        if flag_fdr_p_contrast_15_20 = 1 and flag_fdr_p_contrast_16_20 = 1 then flag_fdr_all = 1; else flag_fdr_all= 0;
...
/* Identify Fusions direction of change for all comparisons */
    data fold;
        set subset;
        fold_CS =  AH_CS_mean_rpkm / AH_MALE_FruM_C__mean_rpkm;
        fold_BER = AH_BerM_mean_rpkm / AH_MALE_FruM_C__mean_rpkm;
...
proc export data=induced
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/exported_data_from_michelle/FruM_C_male_ind_1fold_jmf.tab'
            dbms=CSV replace;
            putnames=no;
            run;

proc export data=repressed
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/exported_data_from_michelle/FruM_C_male_rep_1fold_jmf.tab'
            dbms=CSV replace;
            putnames=no;
            run;

data FRU.fruC_male_fus_ind_rep_nfold;
    set merged2;
    rename flag_induced = flag_fruC_male_ind_nofold;
    rename flag_repressed = flag_fruC_male_rep_nofold;
    keep fusion_id flag_induced flag_repressed;
    run;

proc export data=FRU.fruC_male_fus_ind_rep_nfold
            outfile= '/home/jfear/mclab/arbeitman/arbeitman_fru_network/reports_external/FruM_C_male_flag_ind_rep_nofold.csv'
            dbms=CSV replace;
            putnames=yes;
            run;
