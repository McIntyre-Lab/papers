...
data geno1_2;
   ...
   format sd_temp f8.3;
   set geno1;
   length hmz1_expression_log $30.;
   avg_temp=strip(put(mean, best8.3));
   sd_temp=strip(put(sd, best8.3));
   hmz1_expression_log=catt(avg_temp, ' ± ',sd_temp);
   rename mean=hmz1_mean_log sd=hmz1_sd_log;
...
data geno2_2;
   ...
   format sd_temp f8.3;
   set geno2;
   length htz_expression_log $30.;
   avg_temp=strip(put(mean, best8.3));
   sd_temp=strip(put(sd, best8.3));
   htz_expression_log=catt(avg_temp, ' ± ',sd_temp);
   rename mean=htz_mean_log sd=htz_sd_log;
   drop avg_temp sd_temp;
run;
...
data geno3_2;
   ...
   format sd_temp f8.3;
   set geno3;
   length hmz2_expression_log $30.;
   avg_temp=strip(put(mean, best8.3));
   sd_temp=strip(put(sd, best8.3));
   hmz2_expression_log=catt(avg_temp, ' ± ',sd_temp);
   rename mean=hmz2_mean_log sd=hmz2_sd_log;
   drop avg_temp sd_temp;
run;
...
data mean_coverage_per_genotype_&setnum.;
   ...
   htz_hmz1_ratio_log=htz_mean_log/hmz1_mean_log;
   hmz2_hmz1_ratio_log=hmz2_mean_log/hmz1_mean_log;
...
data eqtl.mean_coverage_per_gt_log;
