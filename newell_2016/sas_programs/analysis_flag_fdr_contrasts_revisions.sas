...
%fdr_mult(ribo.Output_contrasts_apn0_2, probf, fusion_id, 0.20,0.1,0.05);
/*%fdr_mult(contrasts, probf, fusion_id, 0.20, 0.1, 0.05); *for test set;*/


data output_fdr_probf_apn0;
  set fdr_probf;
  run;


/*Now transpose the data; first rename the contrasts*/
proc freq data = output_fdr_probf_apn0;
  table label ;
  run;

data contrast;
  retain fusion_id label;
  set output_fdr_probf_apn0;
  ...
  label FValue = 'f_contrast';
    rename raw_p  = p_contrast
     ...
     if Label = 'Male-Female'                       then contrast = 5;
  run;
...
data p_trans;
    ...
    label p_contrast_5  =   'p_contrast_Male-Female'        ;
  run;
...
%flag_pvals(p_contrast_5, Male-Female)          ;
...
data fdr_trans;
    ...
    label fdr_p_contrast_5  =   'fdr_p_contrast_Male-Female'                ;
  run;
...
%flag_fdrs(fdr_p_contrast_5,Male-Female)            ;


/*Merge everything together*/

data ribo.flag_fdr_contrast_by_fusion_2;
  merge flag_p_contrast_1 flag_p_contrast_2 flag_p_contrast_3 flag_p_contrast_4 
        ...
        flag_p_contrast_5 flag_fdr_p_contrast_5;
  by fusion_id;
run;
