...
flag on/off in ujc count files 
...
merge &design (in=in1) sex.&species._ujc_cnts_sumtr_stack_w0s (in=in2) ; 
by sample ;
if in2 ;
run;
...
set &species._ujc_cnts1 ;
if jxnHash_sumReadNum > 0 then readCnt_on0 = 1; else readCnt_on0 = 0;
if jxnHash_sumReadNum >= 5 then readCnt_on5 = 1; else readCnt_on5 = 0;
if jxnHash_sumReadNum >= 10 then readCnt_on10 = 1; else readCnt_on10 = 0;
if jxnHash_sumReadNum >= 25 then readCnt_on25 = 1; else readCnt_on25 = 0;
run;

data &species._ujc_cnts3 ;
set &species._ujc_cnts2 ;
...
proc sort data= &species._ujc_cnts3 ;
by sex geneID jxnHash ;
run;

/* gene is on if expressed in ge 50% of the reps */
proc means data = &species._ujc_cnts3  noprint ;
    by sex geneID jxnHash  ;
    var readCnt_on&num;
...
data sex.onCall_UJC_&species._on&num.;  
set &species._sex_on&num._sbys ;   
    ...
    keep geneID jxnHash flag_: f m ;


    run ;

proc sort data = sex.oncall_ujc_&species._on&num. ;
by geneID jxnHash ;
run ;
...
data onCall_ujc_&species._flags2;
merge sex.oncall_ujc_&species._on: ;
by geneID jxnHash ;
run;

data onCall_ujc_&species._flags;
set onCall_ujc_&species._flags2;
keep geneID jxnHash
    flag_sex_limited_rc0 flag_both_sex_off_readCnt: flag_both_sex_on_readCnt: flag_Monly_on_readCnt: flag_Fonly_on_readCnt: ;
run ;

data sex.onCall_ujc_&species._flags; 
retain geneID jxnHash flag_sex_limited_rc0 flag_analyzable flag_both_sex_off_readCnt0 flag_both_sex_off_readCnt5 flag_both_sex_off_readCnt10 flag_both_sex_off_readCnt25 
...
set onCall_UJC_&species._flags; 
...
ods pdf file = "!MCLAB/sex_specific_splicing/expression_output/freqs_onCalls_UJC_counts.pdf" ;

%macro which (species) ;
%macro freqs (num) ;

title "&species. MF on calls for jxnHash,  readCnt > &num";
proc freq data = sex.onCall_ujc_&species._on&num. ;
tables flag_:  ;
run;
...
ods pdf file = "!MCLAB/sex_specific_splicing/expression_output/freqs_onCalls_UJC_sexLimited_flagAnalyze.pdf" ;

%macro which (species) ;

title "&species. MF sex limited counts";
proc freq data = sex.onCall_ujc_&species._flags ;
tables flag_sex_limited_rc0 * flag_both_sex_off_readCnt5  ;
run;
/* 368 ujcs that have at least 5 reads in 1 sex and 0 reads in opposite sex 
   of these, there are 28 that have at least 25 reads in 1 sex and 0 reads in opposite sex  (modify above to flag_both_sex_off_readCnt25)

   61296 ujcs that are analyzable (not sex-limited and have at least 5 reads in ge 50% of the reps in at least 1 sex
*/
