...
/*
create design file for UJC containing all combos 
*/
...
data &species._ujc1 ;
set  sex.&species._ujc_cnts_sumTR_stack;
keep jxnHash;
run ;

proc sort data =  &species._ujc1 nodups;
by _all_ ;
run;
...
data &species._ujc_w0 ;
merge sex.&species._ujc_cnts_sumTR_stack &species._ujc2  ;
by sample jxnHash ;
run;

data &species._ujc_cnts_stack_with_0 ;
set  &species._ujc_w0 ;
if jxnHash_sumReadNum = . then jxnHash_sumReadNum = 0;
else jxnHash_sumReadNum = jxnHash_sumReadNum ;
run ;
...
data sex.&species._ujc_cnts_sumtr_stack_w0s ;
retain sample jxnHash ;
set &species._ujc_cnts_stack_with_0 ;
run ;
%mend ;
...
proc freq data = sex.dmel_ujc_cnts_sumtr_stack_w0s ;
tables sample / out = aa_mel;
run;
