...
proc sort data=fru.model_fru_by_fusion_miss;
    by fusion_id;
    run;

proc sort data=fru.all_means_miss;
     by fusion_id;
     run;

proc sort data=fru.flag_fdr_contrast_by_fusion_mis2;
    by fusion_id;
    run;

proc sort data=fru.flag_fail_norm_by_fusion_miss;
    by fusion_id;
    run;

proc sort data=fru.flag_no_var_miss;
    by fusion_id;
    run;

proc sort data=fru.flag_zero_mean_miss;
    by fusion_id;
    run;

proc sort data=fru.flag_missing_miss;
    by fusion_id;
    run;

data fru.results_by_fusion_miss;
      merge dmel530.Fb530_si_fusions_unique_flagged (in=in1)
          fru.model_fru_by_fusion_miss
          fru.all_means_miss
          fru.flag_fdr_contrast_by_fusion_mis2
          fru.flag_fail_norm_by_fusion_miss
...
proc sort data=fru.results_by_fusion_miss;
    by fusion_id;
    run;
...
proc contents data=fru.results_by_fusion_miss varnum; run;

data fru.results_plus_gov2_miss;
    ...
    merge fru.results_by_fusion_miss (in=in1) dmel530.fusions2go; 
    by fusion_id;
    if in1; 
    label df=; label ss=;
