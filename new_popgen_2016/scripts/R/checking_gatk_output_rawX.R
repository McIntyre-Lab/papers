...
#### HETEROZYGOSITY ####
het<-read.table("/home/fnew/dsim/checking_gatk_output/sim_X_biall_het.het", header=TRUE)
attach(het)
het2<- het[order(F),]

plt <- plot(het2$F, ylab="F", main="Coefficient of inbreeding")
...
#### SNP DENSITY ####
xsnp <- read.table("/home/fnew/dsim/checking_gatk_output/10permiss_rawX_10kb.snpden", header=TRUE)
...
points(x=494.34, y=0, pch=17, col="red", cex=2) #mean
points(x=499, y=0, pch="*", col="blue", cex=4) #median

avg <- mean(xsnp$SNP_COUNT)
med <- median(xsnp$SNP_COUNT)

#### PI ####
pi <- read.table("/home/fnew/dsim/checking_gatk_output/10permiss_rawX_10kb.windowed.pi", header=TRUE)

plot(x=pi$BIN_START, y=pi$PI, type="l", main="Nucleotide Diversity on X" , xlab="Position in 10kb windows", ylab="pi", ylim=c(0,0.015))


#### TAJIMA'S D ####
d <- read.table("/home/fnew/dsim/checking_gatk_output/10permiss_rawX_10kb.Tajima.D", header=TRUE)
