...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  7 21:02:19 2024
...
gene_cell_effect_DF=gene_effect_DF[gene_effect_DF['Effect'] == 'population']

import seaborn as sns
import matplotlib.pyplot as plt

filter_prop_001_DF=gene_cell_effect_DF[gene_cell_effect_DF['prop_frags_sig_001']> 0]

filter_prop_00001_DF=gene_cell_effect_DF[gene_cell_effect_DF['prop_frags_sig_00001']> 0]
...
#Save the figure
plt.savefig(inDir+"hist_prop_cellType_effect_frags_DE.png")
plt.show()
plt.close()
...
gene_cell_effect_DF['flag_gene_DE_001'] = (gene_cell_effect_DF['prop_frags_sig_001'] > 0.2).astype(int)

# Flag for proportion_of_frags_sig_0_00001 greater than 0.2
gene_cell_effect_DF['flag_gene_DE_00001'] = (gene_cell_effect_DF['prop_frags_sig_00001'] > 0.2).astype(int)
...
gene_cell_effect_DF['flag_T1D_gene'] = gene_cell_effect_DF['geneID'].isin(t1d_geneDF['ensembl_geneID']).astype(int)
gene_cell_effect_DF['flag_immune_gene'] = gene_cell_effect_DF['geneID'].isin(immune_geneDF['Ensembl_ID']).astype(int)

#Cnt DE genes
cnt_DE_all_001=gene_cell_effect_DF['flag_gene_DE_001'].sum()
cnt_DE_all_00001=gene_cell_effect_DF['flag_gene_DE_00001'].sum()
cnt_DE_immune_001=(gene_cell_effect_DF['flag_gene_DE_001'] * gene_cell_effect_DF['flag_immune_gene']).sum()
cnt_DE_immune_0001=(gene_cell_effect_DF['flag_gene_DE_00001'] * gene_cell_effect_DF['flag_immune_gene']).sum()
cnt_DE_t1d_001=(gene_cell_effect_DF['flag_gene_DE_001'] * gene_cell_effect_DF['flag_T1D_gene']).sum()
cnt_DE_t1d_00001=(gene_cell_effect_DF['flag_gene_DE_00001'] * gene_cell_effect_DF['flag_T1D_gene']).sum()

#Get counts of analyzable genes
cnt_all=len(gene_cell_effect_DF)
cnt_immune=gene_cell_effect_DF['flag_immune_gene'].sum()
cnt_T1D=gene_cell_effect_DF['flag_T1D_gene'].sum()
...
ax[0].set_title('Cell Type Effect - Proportions of DE Genes at 0.001 (Cutoff 0.2)')
ax[0].set_ylabel('Proportion of DE Genes')
ax[0].set_ylim(0, 1)

# Plot for 0.00001
ax[1].bar(categories, proportions_00001, color='lightgreen')
ax[1].set_title('Cell Type Effect - Proportions of DE Genes at 0.00001 (Cutoff 0.2)')
...
#Save fig
plt.savefig(inDir+"prop_genes_DE_cellType_effect.png")
plt.show()
plt.close()
