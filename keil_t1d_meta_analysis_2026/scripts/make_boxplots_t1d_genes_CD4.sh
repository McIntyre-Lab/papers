...
IN_CSV=${IND}/norm_data_frag_CD4_stack.csv
POSITIONS=${PROJ}/featureID_2_ef_start.csv
METRIC=log_uq_apn

OUTD=${PROJ}/boxplots_t1d_genes_CD4
    mkdir -p ${OUTD}
...
while IFS=, read -r SYMBOL ENSG _; do
  ...
  python $PER_FRAG_CC \
      ...
      --gene $ENSG \
      --metric $METRIC \
      --out $GDIR/${ENSG}_${SYMBOL}_per_frag_case_control.pdf
  ...
  python $PER_FRAG_MF \
      ...
      --gene $ENSG \
      --metric $METRIC \
      --out $GDIR/${ENSG}_${SYMBOL}_per_frag_M_F.pdf
  ...
  python $GENE_CC \
     ...
     --metric $METRIC \
     --positions ${POSITIONS} \
     --out $GDIR/${ENSG}_${SYMBOL}_by_gene_case_control.png
  ...
  python $GENE_MF \
      ...
      --metric $METRIC \
      --positions ${POSITIONS} \
      --out $GDIR/${ENSG}_${SYMBOL}_by_gene_M_F.png
