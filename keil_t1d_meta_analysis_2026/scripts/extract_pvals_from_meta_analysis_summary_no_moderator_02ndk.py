...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 25 17:09:47 2024
    ...
    for filename in os.listdir(directory):
        if filename.endswith(suffix):
            ...
            if p_heterogeneity is not None:
                ...
                if len(parts) >= 3:
                    # Assume standard structure: *_GENEID_CELLTYPE_*
                    gene_id = parts[-3]
                    cell_type = parts[-2]
                    ...
                    results.append([gene_id, cell_type, p_heterogeneity])
                else:
                    print(f"⚠️ Warning: Unexpected filename format: {filename}")
            else:
                print(f"⚠️ No p-values found in {filename}")

    return pd.DataFrame(results, columns=['geneID', 'cellType', 'pval_heterogeneity'])
