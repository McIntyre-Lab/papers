...
for(cell in cell_list) {
  ...
  datain$geneID <- sapply(strsplit(datain$featureID, ":"), `[`, 1) 
      ...
      for(gene in unique(geneCol)) {
        ...
        exondf= subset(gene_annot_df, ef_ir_flag == 0)
        
        num_features <- nrow(exondf)
        
        print(paste(cell,gene))
        
        if (num_features > 1 && "M" %in% genedf$sex && "F" %in% genedf$sex) {
        ## (1) fixed-effects meta-analysis - no moderator
        dataout = rma(effect_size, variance, data=exondf,
                      slab=exondf$featureID,
                      measure="MD", method="FE", )
      
        fileSum1 <- file.path(outSum, paste("summary_", gene, "_", cell, "_exonsOnly_FE.txt", sep=""))
        out <- summary(dataout)
        capture.output(out, file = fileSum1)
        
        ## (2) mixed-effects meta-analysis - sex (categorical) as moderator, model = FE
        dataout2 = rma(effect_size, variance, data=exondf, mods = ~ sex, 
                       slab=exondf$featureID,
                       measure="MD", method="FE", )
        
        fileSum2 <- file.path(outSum, paste("summary_sexMod_", gene, "_", cell, "_exonsOnly_FE.txt", sep=""))
        ...
        #dataout3 = rma(effect_size, variance, data=exondf, mods = ~ sex, 
        #               slab=exondf$featureID,
        ...
        outPNG <- paste("forestPlot_", gene, "_", cell, "_exonsOnly_FE.png", sep="")
        png(file.path(outPlot, outPNG), width=960, height=640)
        forest(dataout, header=TRUE, slab=exondf$featureID, order=sex, ilab=sex,
               xlab="For each sex, Case vs Control (MD, FE)")
        dev.off()
 
        ## (2A) forest plot from fixed effects model - sex moderator, FE
        outPNG2 <- paste("forestPlot_sexMod_", gene, "_", cell, "_exonsOnly_FE.png", sep="")
        png(file.path(outPlot, outPNG2), width=960, height=640)
        forest(dataout2, header=TRUE, slab=exondf$featureID, 
        ...
        #forest(dataout3, header=TRUE, slab=exondf$featureID, 
        #       ilab=sex, ilab.xpos=-3.5, order=sex, 
