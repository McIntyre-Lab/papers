...
output out = seq.de_resid_frag_control resid=resid pred=pred student=student ;
ods output 
    tests1 = seq.de_t1_frag_control  
    lsmeans = seq.de_ls_frag_control 
    diffs = seq.de_pd_frag_control_FbyCell  
    slices = seq.de_sl_frag_control_FbyCell ;
    
run;

proc contents data = seq.de_sl_frag_control_FbyCell ; run;
...
outfile = "!MCLAB/t1d_case_control_cellType/quantify_t1d_pacbio_transcripts/model_DE_frag_controls_CD4_CD8/&output..csv"
dbms = csv replace ;
run;
%mend ;

%outputing (de_t1_frag_control, ProbF) ;
%outputing (de_ls_frag_control, Probt) ;
%outputing (de_pd_frag_control_FbyCell, Probt) ;
%outputing (de_sl_frag_control_FbyCell, ProbF ) ;
...
run;
%mend ;
%outputing (de_resid_frag_control) ;
