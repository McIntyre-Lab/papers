...
def parse_options():
    ...
    parser.add_argument("-p", "--output_readid_dir", dest="readid_out", required=True, help="Ouput directory where read ids for categories are stored")
...
def get_category_with_value_one(data_dict):
    for key, value in data_dict.items():
        if value == 1:
            return key
    return None

def readids_to_file(data_dict, out_dir):
    if not os.path.exists:
        os.makedirs(out_dir)
    grouped_data = {}
    for read_id, categories in data_dict.items():
        category = get_category_with_value_one(categories)
        if category:
            if category not in grouped_data:
                grouped_data[category] = []
            grouped_data[category].append(read_id)

    # Write each category's read IDs to a separate file, overwriting existing files
    for category, read_ids in grouped_data.items():
        file_name = out_dir + category + '_read_ids.txt'
        with open(file_name, 'w') as file:  # 'w' mode to overwrite
            for id in read_ids:
                file.write(id + '\n')
    return grouped_data
...
def process_read_counts(features, sama_reads, samb_reads):
    ...
    for read in readkeys:
        ...
        counts_readid[read] = dict(count_template)
        ...
        if a_count == 0 and b_count == 0:
            ...
            if b_count == 1:
                if b_exact == 1:
                    ...
                    counts_readid[read]['b_single_exact'] += 1
                elif b_exact == 0:
                    b_single_inexact += 1
                    counts[read_id]['b_single_inexact'] += 1
                    counts_readid[read]['b_single_inexact'] += 1
            elif b_count > 1:
                if b_exact == 1:
                    b_multi_exact += 1
                elif b_exact == 0:
            ...
            if a_count == 1:
                if a_exact == 1:
                    ...
                    counts_readid[read]['a_single_exact'] += 1
                elif a_exact == 0:
                    a_single_inexact += 1
                    counts[read_id]['a_single_inexact'] += 1
                    counts_readid[read]['a_single_inexact'] += 1
            elif a_count > 1:
                if a_exact == 1:
                    a_multi_exact += 1
                elif a_exact == 0:
                    ...
                    if b_count == 1:
                        if b_exact == 1:
                            ...
                            if a_pos != '' and b_pos != '' and a_pos == b_pos and a_rname == b_rname:
                                ...
                                counts_readid[read]['both_single_exact_same'] += 1
                            else:
                                both_single_exact_diff += 1
                                counts[read_id]['both_single_exact_diff'] += 1
                                counts_readid[read]['both_single_exact_diff'] += 1
                        else: # b_exact == 0
                            a_se_b_si += 1
                            counts[read_id]['a_exact_b_inexact'] += 1
                            counts_readid[read]['a_exact_b_inexact'] += 1
                    else: # b_count > 1
                        if b_exact == 0:
                            a_se_b_mi += 1
                        else:
    ...
    return (totals, counts, counts_readid)
        ...
        if sama_reads:
            ...
            if samb_reads:
                ...
                if features:
                    ...
                    tstart = time.time()
                    totals, counts, counts_readid = process_read_counts(features, sama_reads, samb_reads)
                    logger.debug("Time to calculate read counts: {0:n} seconds".format(time.time() - tstart))
                    # Write read ids to seperate files based on categories
                    # Grouping by a specified category (replace 'your_category_key' with the actual key)
                    #print(counts_readid)
                    readids_to_file(counts_readid, opts.readid_out)
