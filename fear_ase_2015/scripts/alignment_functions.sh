...
function bowtie_se_uniq {
    ...
    bowtie -S \
        ...
        UNALN=UNALN.fq
        cat ${NAME}_unaln_bt.fq > $UNALN 2>/dev/null || ( echo "WARNING: There were no unaligned reads" >> $MYLOG; true )
        cat ${NAME}_ambig_bt.fq >> $UNALN 2>/dev/null || true
        
        check_bowtie

        READS=${NAME}_unaln_bt.fq
        rm $UNALN 2>/dev/null || true
...
function bowtie_pe_uniq {
    ...
    bowtie -S \
        ...
        READS=READS.fq
        cat $READ1 $READ2 > $READS 2>/dev/null || true

        ALN=${NAME}_aln_bt.sam $ALN

        UNALN=UNALN.fq
        cat ${NAME}_unaln_bt_1.fq > $UNALN 2>/dev/null || ( echo "WARNING: There were no unaligned reads" >> $MYLOG; true )
        cat ${NAME}_unaln_bt_2.fq > $UNALN 2>/dev/null || ( echo "WARNING: There were no unaligned reads" >> $MYLOG; true )
        cat ${NAME}_ambig_bt_*.fq >> $UNALN 2>/dev/null || true

        du -sh *.fq >>$MYLOG
        
        check_bowtie
        rm $UNALN 2>/dev/null || true
        rm $READS 2>/dev/null || true
