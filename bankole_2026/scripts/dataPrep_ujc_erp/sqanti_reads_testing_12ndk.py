...
#in_ref = '/nfshome/nkeil/mnt/exasmb.rc.ufl.edu-blue/mcintyre/share/sex_specific_splicing/fiveSpecies_annotations/fiveSpecies_2_dmel6_ujc.gtf'
#ref = '/nfshome/nkeil/mnt/exasmb.rc.ufl.edu-blue/mcintyre/share/references/hg38_ensembl/GRCh38.104/Homo_sapiens.GRCh38.104.gtf'
...
def getOptions():
    ...
    parser.add_argument('-d', '--design', type=str, dest="inDESIGN" ,required=True, help='Path to design file, must have sampleID column, class')
    parser.add_argument('-f', '--factor', type=str, dest="inFACTOR" ,required=False, help='Path to design file, must have sampleID column')
    ...
    parser.add_argument('-je','--jxn-expression', type=int, dest="JXNEXP", required=False, help='Coverage threshold for detected donors and acceptor', default = 3)
    ...
    parser.add_argument('-pj','--perc-junctions', type=int, dest="PERCMAXJXN", required=False, help='Percent of the max junctions in gene for determining near full-length putative anannotated transcripts', default = 80)
...
def proc_samples(design_file, ref):
    # Read design file
    #design_file = '/nfshome/nkeil/Desktop/test/test_sqanti_reads/design_sqanti_testing_02ndk.csv'    
    design_DF = pd.read_csv(design_file, sep=",")
    # design_DF = design_DF[design_DF['species']=='mel']
    
    # design_DF = design_DF.iloc[6:8]
    ...
    class_cols = ['isoform','chrom','strand','exons','associated_gene','associated_transcript','structural_category','subcategory',
                    'length', 'RTS_stage','perc_A_downstream_TTS','ref_length','ref_exons','all_canonical','predicted_NMD']
    ...
    class_dtypes = {'isoform': 'string', 'chrom': 'string', 'strand': 'string', 'exons': 'Int64', 'associated_gene': 'string','associated_transcript': 'string', 
                    ...
                    'ref_length': 'Int64','ref_exons': 'Int64', 'all_canonical': 'string', 'predicted_NMD': 'boolean'}
    ...
    for index, row in design_DF.iterrows():
        ...
        index_file = row['index_file']  #!!! This section will need to be changed
        # JxnHash should be output as part of the class file so there should not be any need to have an index file here
        ...
        # index_DF = ujc_main(gtf)
        # This section will need to be changed
        # JxnHash should be output as part of the class file so there should not be any need to have an index file here
        print("Loading index file: "+ sampleID)
        index_DF = pd.read_csv(index_file, sep=",", usecols = ['transcriptID', 'jxnHash'], low_memory=False)

        ##Merge classification DF, count jxns DF and jxn hash into one
        class_DF = merge_three_dfs(class_DF, count_jxns_DF, index_DF, 'isoform', 'isoform', 'transcriptID')
...
#            )
        ...
        ujc_count_DF = class_DF.groupby('jxnHash').agg({
            ...
            'known_canonical': 'first',  # Assuming the first value represents the group
        ...
        ##Err DFs
        # Information on suspicious reads not in classification file
        # Calculate counts
...
def prep_tables(ref_DF, gene_count_dfs,ujc_count_dfs,length_dfs,cv_dfs, err_dfs, fsm_dfs, ism_dfs, nic_nnc_dfs, nov_can_dfs,length_Dct ):
    ...
    #gene_count_DF['flag_fusion_gene'] = np.where(has_underscore & fusion_non_zero, 1, 0)
...
def identify_cand_underannot(ujc_count_DF, factor_level = None):
...
def main():
    ...
    identify_cand_underannot(ujc_count_DF, factor_level=args.FACTORLVL)
