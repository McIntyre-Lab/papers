...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 24 09:54:11 2025

@author: ammorse

add sexClass where to datafile jxnHash where flag_anno = 1
...
for species,species_data in species_dct.items():
    ...
    # import datafile                            
    datafile = pd.read_csv(f"{proj}/submission/supplementary/datafiles/datafile_jxnHash_{species}_w_annoFlag_ERP_ESP_info_strCat_flagNovel.csv")
    ...
    mask = datafile.flag_jxnHash_in_fiveSpecies_full_anno == 1
    datafile.loc[mask, "sexClass_in_anno"] = tmp[mask]
    
    datafile.loc[datafile.flag_jxnHash_in_fiveSpecies_full_anno == 0, "sexClass_in_anno"] = np.nan
    
    # check with crosstab
    freq = pd.crosstab(datafile['sexClass_in_anno'], datafile['flag_jxnHash_in_fiveSpecies_full_anno'])
    #print(freq)
    ones = freq[1.0].sum()
    print(ones)  ## 22665    
    print(datafile.flag_jxnHash_in_fiveSpecies_full_anno.value_counts()) ## 22665
    ...
    # save to csv
    datafile.to_csv(f"{proj}/submission/supplementary/datafiles/datafile_jxnHash_{species}_w_annoFlag_ERP_ESP_info_strCat_flagNovel_02amm.csv", index=False)    
...
for species,species_data in species_dct.items():
    ...
    print(datafile.flag_jxnHash_in_fiveSpecies_full_anno.value_counts()) ## 17937
