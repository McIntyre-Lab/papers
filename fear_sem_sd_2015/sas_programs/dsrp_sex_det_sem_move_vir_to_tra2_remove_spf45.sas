...
/* Move vir To Tra2 Model without Spf45 */
    options ps = 70 ls=80;
    proc printto LOG='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/logs/move_vir_to_tra2_no_spf45_estimates.log' PRINT='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/move_vir_to_tra2_no_spf45_estimates.lst' NEW;
    run;

    proc calis data=w_sample method=fiml COVARIANCE RESIDUAL MODIFICATION maxiter=10000;
        path
            fl_2_d_A fl_2_d_B snf -> Sxl_A,
    ...
    * print log and output back to listings;
    ...
    proc export data=combined outfile='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/move_vir_to_tra2_no_spf45_estimates.csv' dbms=csv replace;
    putnames=yes;
    run;
