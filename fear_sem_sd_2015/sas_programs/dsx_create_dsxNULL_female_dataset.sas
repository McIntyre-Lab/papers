...
/* Combine means and fdr flags */
    ...
    data mydata;
        merge dmel530.Fb530_si_fusions_unique_flagged (in=in1) dsx.exp_means (in=in2) 
...
/* Create A subset that removes un-analyzed fusions and multigene fusions */
    data subset;
        set mydata;
        if flag_fdr_p_contrast_2_05 = ' ' then delete;
        if flag_fdr_p_contrast_9_05  = ' ' then delete;
        where FBgn_cat ^? ';';
        run;

/* Identify Fusions that have sig FDR for all comparisons */
    * BerF-dsxNullF(2), CSF-dsxNullF(9)
    ;
    data flag_fdr_all;
        set subset;
        if flag_fdr_p_contrast_2_05 = 1 and flag_fdr_p_contrast_9_05 = 1 then flag_fdr_all = 1; else flag_fdr_all= 0;
...
/* Identify direction comparisons */
    data direction;
        set subset;
        dir_BER =  AH_BerF_mean_rpkm / AH_dsxNullF_mean_rpkm;
        dir_CS = AH_CSFemale_mean_rpkm / AH_dsxNullF_mean_rpkm;
