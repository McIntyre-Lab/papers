...
* entire DSRP dataset.
********************************************************************************/
* I could not get this to export all of the columns (7424) in SEM.dsrp_sbs_gene_level_sym. So I ended up
* exporting the file to a csv from JMP genomics. When saving under options select csv.
*
* Saved SEM.dsrp_sbs_gene_level_sym /home/jfear/tmp/dsrp_sbs_gene_level_sym.txt;

/* Run GGM on CEGS Virgin */
data _null_;
    call system('Rscript $MCLAB/cegs_sem_sd_paper/r_programs/cegsV_ggm_gene.R');
    run;
