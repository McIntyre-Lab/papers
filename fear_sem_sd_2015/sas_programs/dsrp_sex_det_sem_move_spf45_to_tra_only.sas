...
/* Add Spf45 link to only fru, suggested by residuals */
    options ps = 70 ls=80;
    proc printto LOG='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/logs/move_spf45_to_tra_only.log' PRINT='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/move_spf45_to_tra_only.lst' NEW;
    run;

    proc calis data=w_sample method=fiml COVARIANCE RESIDUAL MODIFICATION maxiter=10000;
        path
            fl_2_d_A fl_2_B snf vir -> Sxl_A,
            fl_2_d_A fl_2_B vir Sxl_A Spf45 -> tra,
    ...
    * print log and output back to listings;
    ...
    proc export data=combined outfile='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/move_spf45_to_tra_only.csv' dbms=csv replace;
    putnames=yes;
    run;
