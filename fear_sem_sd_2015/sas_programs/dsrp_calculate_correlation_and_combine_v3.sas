...
/* Use a python script to combine highly correlated isoforms within gene */
    %macro combine_by_corr(isoform);
        %if %varexist(dsrp_sbs_symbol, &isoform) %then %do;
            data subset;
                set dsrp_sbs_symbol;
        ...
        data _null_;
            command = 'python $MCLAB/cegs_sem_sd_paper/scripts/clusterByCorr.py -c 0.75 -i /tmp/corr_subset.csv -o $HOME/tmp/analysis_output/correlation/' || "&isoform" ||'.csv -t $HOME/tmp/analysis_output/correlation/' || "&isoform" ||'_iso_table.csv';
            call system(command);
            run;
    %mend;
