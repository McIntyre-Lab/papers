...
/* Add Spf45 link to only fru, suggested by residuals */
    options ps = 70 ls=80;
    proc printto LOG='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/logs/spf45_add_only_fru_estimates.log' PRINT='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/spf45_add_only_fru_estimates.lst' NEW;
    run;
    ...
    * print log and output back to listings;
    ...
    proc export data=combined outfile='!MCLAB/cegs_sem_sd_paper/analysis_output/sem/spf45_add_only_fru_estimates.csv' dbms=csv replace;
    putnames=yes;
    run;
