...
data clean1;
merge ortho.cvrg_fusion_simref_stack(in=in1) dsgn_file (in=in2);
by sampleID ;
run;

proc sort data = clean1 ;
by featureID ;
proc sort data=ortho.oncall_sample_apn0_simRef;
by featureID;
run;

data clean2;
merge clean1 (in=in1) ortho.oncall_sample_apn0_simRef (in=in2);
by featureID ;
run;
...
/* Calculate With-in sample basic statistics */
    ...
    /* Summarize statistics to experiment level */
        ...
        /* check q3 by sample -- NOTE: if remove sim_12_m_noEtoh_rep2 q3 = 56.22 WITH sim_12_m_noEtoh_rep2 q3 = 56 
            q3 for sim_12_m_noEtoh_rep2 is 17.91 --> this is LOW compared to others
...
*/
    ...
    /* make perm */
data ortho.norm_stats_fusion_simRef ;
set stack_uq ;
run;
