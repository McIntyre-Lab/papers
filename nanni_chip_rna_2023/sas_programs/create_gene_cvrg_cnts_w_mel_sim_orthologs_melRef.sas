...
data cc_mel_stack ;
set ortho.cvrg_gene_wt_log_uq_apn_melRef ;
if find(sampleID, "mel") ge 1 ;
rename geneID = mel_geneID ; /* mel aligned to mel ref */
run ;

data cc_sim_stack ;
set ortho.cvrg_gene_wt_log_uq_apn_melRef ;
if find(sampleID, "sim") ge 1 ;
rename geneID = mel_geneID ; /* sim aligned to mel ref */
run ;

proc freq data = cc_mel_stack ;
tables mel_geneID / out = cnts_mel ;
run; /* 11067 genes, 24 obs per gene  */
proc freq data = cc_sim_stack ;
tables  mel_geneID / out = cnts_sim ;
run; /* 11067 genes, 23obs per gene (dropped 1 sample */
...
data ortho.cvrg_norm_gene_ortho_2_mel ;  /* stacked dataset */
set cvrg_gene_mel_sim_orthologs ;
run;


/* make wide for log_ug_apn and log_ug_apn_noZero */
proc contents data = ortho.cvrg_norm_gene_ortho_2_mel ; run;

proc sort data = ortho.cvrg_norm_gene_ortho_2_mel ; 
by melID_simID ;
run;

%macro flip (var, shrt) ;

proc transpose data =ortho.cvrg_norm_gene_ortho_2_mel out = wide_&var. ; 
by melID_simID ;
id sampleID ;
var &var. ;
run;

data ortho.cvrGene_ortho2mel_&shrt. ;
set wide_&var. ;
drop _name_ ;
run ;

proc export data = ortho.cvrGene_ortho2mel_&shrt.
outfile = "/nfshome/ammorse/mclab/SHARE/McIntyre_Lab/Dros_CHIP_RNA_ms/RNAseq/Coverage_counts/gene_cvrg_cnts_ortho/cvrGene_ortho2mel_&shrt..csv"
dbms = csv replace;
run ;
