/*******************************************************************************
* Filename: cegsV_dist_models_better_than_baseline_nofilter_yp2_BIC12_no_dsx.sas
*
...
/* Figure out what the baseline BIC was */
    ...
    * putting into a user variable.
    ;
    proc sort data=SEM.cegsV_ag_yp2_stack_bic_no_dsx;
        by gene BIC;
        run;

    data _null_;
        * since I sorted by gene the first 38 obs should be all 38 models for the first gene;
        set SEM.cegsV_ag_yp2_stack_bic_no_dsx (obs = 38); 
...
/* Flag Model if they were at least 12 BIC less than baseline */
    data flag_add;
...
/* Export dataset */
    proc export data=freqdist_w_path2 
        outfile='!MCLAB/cegs_sem_sd_paper/analysis_output/adding_genes/cegsV_ag_yp2_freq_a_model_was_better_BIC12_no_dsx.csv' 
        dbms=csv replace;
        putnames=yes;
        run;
