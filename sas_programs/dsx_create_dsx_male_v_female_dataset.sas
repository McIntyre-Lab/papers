...
/* Create A subset that removes un-analyzed fusions and multigene fusions */
    data subset;
        set mydata;
        if flag_fdr_p_contrast_1_05 = ' ' then delete;
        if flag_fdr_p_contrast_7_05  = ' ' then delete;
        where FBgn_cat ^? ';';
        run;

/* Identify Fusions that have sig FDR for all comparisons */
    * BerF-BerM(1), CSF-CSM(7)
    ;
    data flag_fdr_all;
        set subset;
        if flag_fdr_p_contrast_1_05 = 1 and flag_fdr_p_contrast_7_05 = 1 then flag_fdr_all = 1; else flag_fdr_all= 0;
...
/* Identify direction comparisons */
    data direction;
        set subset;
        dir_BER =  AH_BerM_mean_rpkm / AH_BerF_mean_rpkm;
        dir_CS = AH_CS_mean_rpkm / AH_CSFemale_mean_rpkm;
...
/* Merge on FBgn_cat and collapse to gene level */
    ...
    data SEM.dsx_ctrl_female_induced;
        set sum;
    ...
    data SEM.dsx_ctrl_female_repressed;
        set sum;
