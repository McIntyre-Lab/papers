...
libname addgen '!MCLAB/cegs_sem_sd_paper/adding_genes/sas_data';
...
proc transpose data=combined_uniq out=flip;
    by gene;
    var BIC;
    id model;
    run;
...
data SEM.dsrp_adding_genes_sbs_bic;
    set sbs;
    run;

/* Create a Stacked version just in case */
proc transpose data=SEM.dsrp_adding_genes_sbs_bic out=flip;
    by gene;
    var baseline Model:;
    run;

data SEM.dsrp_adding_genes_stack_bic;
    set flip;
    rename _name_ = model;
    rename col1 = BIC;
    label _name_ = ' ';
