/*******************************************************************************
* Filename: dspr_simulation_make_all_list.sas
*
...
/* Calculate mean and variance for all genes */
    proc sort data=SEM.dsrp_stack_gene_level_sym;
        by symbol;
        run;

    proc means data=SEM.dsrp_stack_gene_level_sym  noprint;
        by symbol;
        output out=all_summary mean(exp)=mean var(exp)=variance;
        run;

    ods graphics on;
    ods pdf file='!MCLAB/cegs_sem_sd_paper/analysis_output/simulation/dspr_all_genes_distributions.pdf';
...
/* Export mean and variances */
    proc export data=all_summary2 outfile='!MCLAB/cegs_sem_sd_paper/analysis_output/simulation/dspr_all_mean_and_variances.csv' dbms=csv replace;
        putnames=yes;
        run;
