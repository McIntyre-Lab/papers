...
/* Combine paternal and maternal RIL ids into a single sample id */
    data w_sample;
        set SEM.cegsV_by_gene_sex_det_sbs;
        run;
...
/* Perform Factor Analisys */
    ...
    proc factor data=w_sample simple method=PRINCIPAL 
        scree rotate=varimax nfact=13 round flag=.4 outstat=factor;
        var : ;
        run;
...
/* Pull out factors and create a summary dataset */
    ...
    data flag_factor;
        ...
        factor_max = max(Factor1,Factor2,Factor3,Factor4,Factor5, Factor6, Factor7, Factor8, Factor9, Factor10, Factor11, Factor12, Factor13);
    ...
    proc export data=flag_factor outfile='!MCLAB/cegs_sem_sd_paper/analysis_output/factor/cegsV_sex_det_factor_analysis_gene.csv' dbms=csv replace;
        putnames=yes;
        run;
...
data _null_;
    call system('python $MCLAB/cegs_sem_sd_paper/scripts/plot_fa_grid.py --factor $MCLAB/cegs_sem_sd_paper/analysis_output/factor/cegsV_sex_det_factor_analysis_gene.csv -o $MCLAB/cegs_sem_sd_paper/analysis_output/factor/cegsV_sex_det_factor_analysis_gene.png -fs 12 -cs 20');
    run;
