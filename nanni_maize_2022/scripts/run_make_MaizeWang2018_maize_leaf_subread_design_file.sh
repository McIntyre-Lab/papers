...
OUTFILE=${DESIGN}/df_MaizeWang2018_maize_leaf_subread.csv
NOHEADER=${DESIGN}/df_MaizeWang2018_maize_leaf_subread_noHeader.csv

## Extract from best guess file the pool 1 sburead fullpath and filename
awk -F "," '$4==1||NR==1{print $1","$2}' ${INFILE} > ${OUTFILE}
awk -F "," '$4==1{print $1","$2}' ${INFILE} > ${NOHEADER}
