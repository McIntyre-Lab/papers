...
def calc_coverage(bdict,num_mapped_reads,read_length):
    ...
    for feature in bdict:
        ...
        if depth != 0:
            ...
            bdict[feature]['rpkm'] = (1000000000.0 * bdict[feature]['reads_in_region']) / float(num_mapped_reads * bdict[feature]['region_length']) # Calculate reads per kilobase per million mapped reads RPKM from Moretzavi et al. 
...
def writeOutput(args,bdict,num_mapped_reads,read_length):
    ...
    if args.cv:
        header = ['featureID','mapped_reads','read_length','region_length','region_depth','reads_in_region','apn','rpkm','mean','std','cv']
        ...
        header = ['featureID','mapped_reads','read_length','region_length','region_depth','reads_in_region','apn','rpkm']
        with open(args.out, 'wb') as OUT:
            OUT.write(','.join(header) + '\n')
            for key in bdict:
