...
### Set Directories
    ...
    wc -l ${PREFIX}.cluster_report.csv | awk -v name=$NAME '{ print name  "\t" $0-1}'  > $OUT/${NAME}_tofu2_mo17_cau_counts_cluster_report.txt

    ## Count unique isoforms from collapse_isoforms_by_sam.py
    wc -l ${PREFIX}.collapsed.group.txt | awk -v name=$NAME '{print name "\t" $1}' > $OUT/${NAME}_tofu2_mo17_cau_counts_collaped_group.txt
