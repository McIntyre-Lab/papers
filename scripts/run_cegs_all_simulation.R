...
allDat <- read.csv(paste0(proj,'/cegs_adding_genes_simulation/all_mean_and_variances.csv'), colClasses=c('NULL', 'numeric', 'numeric'), header=TRUE)
...
simAll <- function(mydat, numGenes){
    ...
    simDat <- apply(simDist, 1, function(x) rnorm(numSamples, x[1], sqrt(x[2])))
    return(simDat)
}
