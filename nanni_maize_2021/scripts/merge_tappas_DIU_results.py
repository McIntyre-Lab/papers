...
#    geneDF['gene_id'].isna().any()
    ...
    for genotype in ["B73","C123","Hp301","Mo17","NC338"]:
        tempDF = pd.read_csv("{}/{}_tappAS_DIUGene_Transcripts.tsv".format(args.inDir,genotype),sep="\t")
        ...
        tempDF = tempDF[['gene_id','DIU_qval_'+genotype,'flag_DIU_'+genotype,'flag_DIU_majorIsoformSwitch_'+genotype,
                         'total_usage_change_'+genotype,'mean_TPM_ambient_'+genotype,'mean_TPM_ozone_'+genotype]]
...
#        tempMerge['merge_check'].value_counts()
    ...
    # Print out counts
    print("\nNumber of DIU genes with major isoform switch detected in at least one treatment of each genotype:\n{}".format(
    ...
    outName = ".".join(args.outFile.split(".")[:-1]) + "_atLeast1geno_DIU_majorIsoformSwitch.csv"
    mergeDF[mergeDF['sum_detect_DIU_majorIsoformSwitch']>0].to_csv(outName,index=False)
