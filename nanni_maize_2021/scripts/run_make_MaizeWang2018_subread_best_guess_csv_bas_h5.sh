...
OUTFILE=${DESIGN}/MaizeWang2018_subread_best_guess_bas_h5.csv

echo "fullpath,moviename,UFbestGuess_LID,UFbestGuess_pool,UFbestGuess_insertSize,UFbestGuess_barcodeID,UFbestGuess_species,UFbestGuess_tissue" > ${OUTFILE}


for FULLPATH in $(find $DATA -name '*.bas.h5'); do
    FILE=$(basename ${FULLPATH} .bas.h5)
    LIDGUESS=none
